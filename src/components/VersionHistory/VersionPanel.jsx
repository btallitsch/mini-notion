...
export default function VersionPanel({ docId }) {
  ...
  return (
    <div style={{ width: 272, background: '#181825', borderLeft: '1px solid #313244',
      display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #313244',
        display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: '#cdd6f4', fontWeight: 700, fontSize: 13 }}>⏰ Version History</span>
        <button onClick={toggleVersionHistory}
          style={{ background: 'none', border: 'none', color: '#6c7086', cursor: 'pointer', fontSize: 20 }}>×</button>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
        {versions.length === 0 ? (
          ...
          <div key={v.id} style={{ marginBottom: 8, padding: 12, background: '#1e1e2e', borderRadius: 10, border: '1px solid #313244' }}>
            ...
            <div style={{ color: '#6c7086', fontSize: 11, marginBottom: i > 0 ? 8 : 0 }}>
              {v.content.replace(/[#*_`>-]/g, '').slice(0, 90)}…
            </div>
            {i > 0 && (
              <button
                onClick={() => { if (window.confirm('Restore this version? Current content will be overwritten.')) restoreVersion(docId, v.id); }}
                style={{ width: '100%', padding: '5px', background: '#313244', border: 'none',
                  borderRadius: 7, color: '#cba6f7', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}
